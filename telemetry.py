...
def main():
    try:
        ...
        while(True):
            ...
            try:
                UsedRam, FreeMemory = mem_data()
            except Exception as e:
                UsedRam = None
                FreeMemory = None
                LOGGER.info(f"memory data error: {e}")
            ...
            # GET BATTERY TEMPS
            try:
                BatteryTemp1 = temperature.sensor_temperature(0x4a)
                LOGGER.info("got battery1 temperature info")
            except Exception as e:
                BatteryTemp1 = None
                LOGGER.info(f"temp sensor 1 error: {e}")
            try: 
                BatteryTemp2 = temperature.sensor_temperature(0x4b)
            ...
            try:
                with open('imu_data/recent_imu.csv', 'r') as csvfile:
                    csv_reader = csv.reader(csvfile)
                    for row in csv_reader:
                        data_dict["GyroX"] = row[1]
                        data_dict["GyroY"] = row[2]
                        data_dict["GyroZ"] = row[3]
                        data_dict["AccelX"] = row[4]
                        data_dict["AccelY"] = row[5]
                        data_dict["AccelZ"] = row[6]
                        data_dict["MagX"] = row[7]
                        data_dict["MagY"] = row[8]
                        data_dict["MagZ"] = row[9]
            ...
            try:
                with open('bme_data/recent_bme.csv', 'r') as csvfile:
                    csv_reader = csv.reader(csvfile)
                    for row in csv_reader:
                        data_dict["BMETemp"] = float(row[1])
                        data_dict["BMEPressure"] = float(row[2])
                        data_dict["BMEHumidity"] = float(row[3])
            ...
            try:
                with open('mag_data/recent_mag.csv', 'r') as csvfile:
                    ...
                    for row in csv_reader:
                        # pdb.set_trace()
                        data_dict["Mag1X"] = row[1]
                        data_dict["Mag1Y"] = row[2]
                        data_dict["Mag1Z"] = row[3]
                        data_dict["Mag2X"] = row[4]
                        data_dict["Mag2Y"] = row[5]
                        data_dict["Mag2Z"] = row[6]
            ...
            # WRITE TO CSV 
            write_line(data_dict, csv_file, GPS_SNR)
            ...
            if (time.time() - starttime > 600):
                ...
                filename = f"telemetry_{date}"
                csv_file = create_file(filename) # returns a path to the CSV file we need to write to 
                LOGGER.info("Created new telemetry file")
...
def write_line(data_dict, csv_file, gps_snr):
    with open(csv_file, mode='a') as file:
        writer = csv.writer(file)
        new_data = list(data_dict.values())
        new_data.append(gps_snr)
...
def mem_data():
    ...
    return round(used_mem/total_mem * 100, 5), free_mem
...
def storage_data():
    ...
    return int(free_storage) * 1024 # store in bytes 
...
def create_file(filename):
    ...
    with open(csv_file, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(
            ...
            "GPSLat", "GPSLatNS", "GPSLn", "GPSLonEW","GPSAlt", "GPSVelocity","GPSFixQual", "Mag1X","Mag1Y","Mag1Z","Mag2X", "Mag2Y", "Mag2Z", "GPSSNR"])
    return csv_file
...
def create_beacon_data(data_dict):
    ...
    with open(csv_file, 'w', newline='') as csvfile:
        ...
        for data in new_data:
            csvwriter.writerow([0 if data == None else data])
