...
mxlgs_doppler_correction_cc_sptr mxlgs_make_doppler_correction_cc(double sampling_freq, unsigned int port, int dir, double int_freq, long int rxfreq)
{
	return mxlgs_doppler_correction_cc_sptr(new mxlgs_doppler_correction_cc(sampling_freq, port, dir, int_freq, rxfreq));
}

/*----------------------------------------------------
*		CONSTRUCTOR
*---------------------------------------------------*/
mxlgs_doppler_correction_cc::mxlgs_doppler_correction_cc(double sampling_freq, unsigned int port, int dir, double int_freq, long int rxfreq) : gr::sync_block("doppler_correction_cc", gr::io_signature::make(1, 1, sizeof(gr_complex)), gr::io_signature::make(1, 1, sizeof(gr_complex))), d_sampling_freq(sampling_freq), d_port(port), d_dir(dir)
{
	d_int_freq = int_freq;
	...
	pthread_create(&socket_thread, 0, &(mxlgs_doppler_correction_cc::start_thread), this);
...
*		WORK FUNCTION
*---------------------------------------------------*/
int mxlgs_doppler_correction_cc::work(int noutput_items,
...
void mxlgs_doppler_correction_cc::socket_thread_work()
{
...
int mxlgs_doppler_correction_cc::process_command(char *command_in, char *response)
{
	...
	case 'F':
		...
		// d_doppler = (float) radio_freq - d_rxfreq + d_int_freq;
		// d_local_freq = d_doppler;

		// For B210
		d_doppler = (float)radio_freq - d_rxfreq;
		d_local_freq = -d_doppler * d_dir + d_int_freq;

		cout << "Doppler: " << d_doppler << endl;
		cout << "\tRequested :" << radio_freq << endl;
		cout << "\tInitial freq: " << d_rxfreq << endl;
